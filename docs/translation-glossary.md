# English–Indonesian Editorial Glossary

This glossary keeps recurring concepts consistent without forcing literal
sentence-by-sentence translation. Meaning, evidence, uncertainty, and safety take
priority over matching English word order.

## Voice and style

- Address the reader as **Anda**. Use a calm, direct, non-prescriptive voice.
- Prefer familiar Indonesian where it remains precise. Introduce a technical
  term only when it helps readers recognize the idea elsewhere.
- Keep titles of cited works, author names, publisher names, and source URLs in
  their published form.
- Preserve qualifications such as *may*, *can*, *is associated with*, and
  *evidence remains mixed*. Do not strengthen them into certainty.
- Local examples may change, but they must illustrate the same mechanism and
  must not introduce an unsupported factual claim.

## Preferred terms

| English concept | Preferred Indonesian | Editorial note |
| --- | --- | --- |
...
For every source/translation pair, confirm that:

1. Every material claim, number, limitation, and safety statement has an
   equivalent meaning.
2. `category`, `difficulty`, `evidence_level`, `confidence`,
   `human_101_order`, `related`, and source URLs remain aligned.
3. Indonesian prose reads naturally without copying English syntax.
4. Search aliases reflect questions an Indonesian reader would actually ask.
5. `source_revision` matches the English `content_revision`.
6. AI assistance is disclosed and the edition receives an independent review.

This is a living glossary. Change it when a clearer term emerges, then check all
published uses before adopting the new preference.
